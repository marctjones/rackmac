...
(define (skip-sp s i end) (if (and (< i end) (space-or-tab? (string-ref s i))) (skip-sp s (add1 i) end) i))
(define (skip-spnl s pos end)
  (define a (skip-sp s pos end))
  (if (and (< a end) (eqv? (string-ref s a) #\newline)) (skip-sp s (add1 a) end) a))
       ...
       (cond
         ...
         [else
          ...
          (cond
            ...
            [else (loop (add1 j))])]))]
    [else
     ;; Scan to the destination's end (or #f), then decide.
     (define-values (j depth)
       (let loop ([j pos] [depth 0])
         (cond
           [(>= j end) (values j depth)]
           [else
            (define c (string-ref s j))
            (cond
              [(and (eqv? c #\\) (< (add1 j) end) (ascii-punctuation? (string-ref s (add1 j))))
               (loop (+ j 2) depth)]
              [(eqv? c #\()
               (if (>= depth 32) (values #f depth) (loop (add1 j) (add1 depth)))]
              [(eqv? c #\)) (if (= depth 0) (values j depth) (loop (add1 j) (sub1 depth)))]
              [(or (eqv? c #\space) (ascii-control? c)) (values j depth)]
              [else (loop (add1 j) depth)])])))
     (cond
       [(or (not j) (not (= depth 0))) (values #f pos)]
